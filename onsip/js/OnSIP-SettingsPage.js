...
function getOnsipUser (callback) {
  ...
  chrome.extension.sendRequest(
    { verifyOnSipUser : true, username : username, password : password},
    function (response) {
	    if (response.ok){
		    if(callback){
		      callback.onSuccess();
...
function validateHighriseCredentials (callback){
  ...
  chrome.extension.sendRequest(
    { verifyHighrise : true, highrise_url : url, highrise_token : token},
    function (response) {
      if (response.ok) {
        if (callback) {
          callback.onSuccess();
...
function validateZendeskCredentials (callback) {
  ...
  chrome.extension.sendRequest(
    { verifyZendesk : true, zendesk_url : url, zendesk_usr : usr, zendesk_pwd : pwd},
    function (response) {
	    if (callback) {
		    if (response.ok) {
		      callback.onSuccess();
